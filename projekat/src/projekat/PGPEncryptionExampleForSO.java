...
public class PGPEncryptionExampleForSO {
	/**
	 * Create an encrypted data blob using an AES-256 session key and the passed in
	 * public key.
	 *
	 ...
	 */
	public static byte[] createEncryptedData(PGPPublicKey encryptionKey, byte[] data) throws PGPException, IOException {
	...
	/**
	 * Extract the plain text data from the passed in encoding of PGP encrypted
	 * data. The routine assumes the passed in private key is the one that matches
	 * the first encrypted data object in the encoding.
	 *
	 ...
	 */
	public static byte[] extractPlainTextData(PGPPrivateKey privateKey, byte[] pgpEncryptedData)
	...
	private static void elgamalExample() throws Exception {
		byte[] msg = Strings.toByteArray("Hello, world!");
		KeyPairGenerator kpGen = KeyPairGenerator.getInstance("DH", "BC");
		kpGen.initialize(2048);
		KeyPair kp = kpGen.generateKeyPair();
		PGPKeyPair elgKp = new JcaPGPKeyPair(PGPPublicKey.ELGAMAL_ENCRYPT, kp, new Date());
		byte[] encData = createEncryptedData(elgKp.getPublicKey(), msg);
		byte[] decData = extractPlainTextData(elgKp.getPrivateKey(), encData);
		System.out.println("elgamal encryption msg length: " + msg.length + " enc.length: " + encData.length
				+ " dec.length: " + decData.length);
		System.out.println(Strings.fromByteArray(decData));
	}

	private static void ecExample() throws Exception {
		byte[] msg = Strings.toByteArray("Hello, world!");
		...
		System.out.println("ec encryption msg length: " + msg.length + " enc.length: " + encData.length
				+ " dec.length: " + decData.length);
		System.out.println(Strings.fromByteArray(decData));
	}
	...
	public static void main(String[] args) throws Exception {
		...
		// you need the two files bcpg-jdk15on-165.jar and bcprov-jdk15to18-165.jar to
		// run the example
